...
async def test_private_key_pulled_from_vault_not_plaintext():
    ...
    assert isinstance(captured["client_keys"], list)
    assert b"BEGIN PRIVATE KEY" in captured["client_keys"][0]
...
async def test_password_auth_passes_password_without_private_keys():
    """auth_type=password 时,连接参数只带保险箱中的 password。"""
    store = LocalSecretStore(master_key=generate_master_key())
    ...
    # 密码来自保险箱;绝不传私钥参数(否则 asyncssh 会尝试私钥认证)
    assert captured["password"] == "s3cr3t-pw"
    assert "client_keys" not in captured
    assert captured["username"] == "ops"


async def test_key_auth_is_default_and_passes_client_keys():
    """不指定 auth_type 时默认私钥认证:连接参数带 client_keys。"""
    store, cred_id = _store_with_key()
    captured: dict[str, object] = {}
...
def test_key_auth_options_are_accepted_by_asyncssh():
    """共享连接参数必须使用 AsyncSSH 2.24 可接受的私钥选项。"""
    private_key = asyncssh.generate_private_key("ssh-ed25519").export_private_key()
    store = LocalSecretStore(master_key=generate_master_key())
    cred_id = store.put("ssh-key", private_key.decode())
    captured: dict[str, object] = {}
    ...
    options = asyncssh.SSHClientConnectionOptions(**captured)
